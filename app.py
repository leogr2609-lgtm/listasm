...
def check_password():
    ...
    # Pantalla de acceso: layout simple y centrado usando componentes nativos.
    # No cambia la lógica de autenticación (solo mejora la presentación).
    col_left, col_center, col_right = st.columns([1, 1.1, 1])

    with col_center:
        with st.container(border=True):
            # Logo/placeholder + mensaje de valor
            st.write("🍏")
            st.subheader("Nutribere Studio")
            st.caption("Listas de supermercado inteligentes para planes nutricionales.")
            st.caption("Ahorra tiempo, evita errores y entrega resultados profesionales.")

            # Formulario simple y legible
            ...
            st.caption("© 2026 Nutribere Studio")
